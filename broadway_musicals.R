...
grosses_top_10 %>% 
  ...
  geom_text(
    ...
    # this geom gets a new data layer: median date per show, so
    # show names can be allocated in the middle of the distribution
    # also we get the labeled total revenue from this layer
    data = grosses_top_10 %>% 
      group_by(show) %>% 
